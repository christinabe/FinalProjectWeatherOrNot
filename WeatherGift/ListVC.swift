...
class ListVC: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ToPageVC" {
            ...
            // This is passed bacy to PageVC as currentPage, so that 
...
extension ListVC: UITableViewDataSource, UITableViewDelegate {
    ...
    //called upon when a cell is tapped
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        //first make copy of item that u will move
        let itemToMove = locationsArray[sourceIndexPath.row]
        //delete item from oroginal location
        locationsArray.remove(at: sourceIndexPath.row)
        //insert item into the "to, post move location
        locationsArray.insert(itemToMove, at: destinationIndexPath.row)
    }
    
    //MARK: - TableView Code to  Freeze the first cell call - no deleting or moving
